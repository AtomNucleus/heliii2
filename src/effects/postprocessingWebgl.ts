...
export function createWebGLPostProcessing(
  ...
  const applyQuality = (q: QualitySettings) => {
    ...
    gradeEnabled = q.colorGrade;
    grade.enabled = q.colorGrade;
    vignetteEnabled = q.vignette;
    vignette.enabled = q.vignette;
    rgbEnabled = q.chromaticAberration;
    rgbShiftPass.enabled = q.chromaticAberration;
    filmEnabled = q.filmGrain;
  ...
  const update = (dt: number) => {
    ...
    if (rgbEnabled) {
      rgbShiftPass.uniforms['amount'].value = 0.00035 + punch * 0.0028;
      rgbShiftPass.uniforms['angle'].value = filmTime * 0.15;
    }
